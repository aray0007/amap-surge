...
2026.05.25 Minis 修改：
- 多账号保存/执行上限改为 10 个
- 新增 nodeseek_max_accounts 配置，默认 10，可自行调整
...
// 最多保存/执行账号数：原版常见配置为 7，这里改为 10；也可通过环境变量/BoxJS nodeseek_max_accounts 自定义
...
//------------------------------------------
async function main() {
    // 顺序执行所有用户，最后统一通知；避免每个账号单独通知导致 Surge 60s 左右超时，只跑到 7 个
    $.notifyMsg = [], $.title = "";
    for (let user of $.userList) {
        try {
            ...
            DoubleLog(`[账号${user.index} ${user.userName || ''}][ERROR]${e}`);
        }
    }
...
class UserInfo {
    ...
    async userAccount() {
        try {
            ...
            let res = await this.fetch({ ...opts, timeout: 30000 });
            return res?.detail;
        } catch (e) {
            this.ckStatus = false;
...
async function checkEnv() { try { if (!userCookie?.length) throw new Error("no available accounts found"); const accounts = userCookie.slice(0, maxAccounts); $.log(`\n[INFO]检测到 ${userCookie?.length ?? 0} 个账号，本次执行 ${accounts.length} 个账号，上限 ${maxAccounts} 个\n`), $.userList.push(...accounts.map((o => new UserInfo(o))).filter(Boolean)) } catch (o) { throw o } }

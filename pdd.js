...
function isHomeRecommendUrl(u) {
  return /page_id=index_list\.html|page_sn=10002|scene=homegoods/i.test(u);
}

function isPersonalRecommendUrl(u) {
  return /personal|center|profile|my|user|mine|page_id=(personal|center|my|profile|user|mine)/i.test(u);
}
...
function stripRedDots(obj) {
  try {
    let s = JSON.stringify(obj)
      .replace(/"number"\s*:\s*\d+/g, '"number":0')
      .replace(/"tab_number"\s*:\s*\d+/g, '"tab_number":0');

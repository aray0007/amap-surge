...
const scriptName = 'WePhone';
const storeKey = 'wephone_accounts_v1';
...
function buildUA(baseUA, seed) {
  ...
  return `WePhone/4.6.4 (com.innovation.wephone; build:18; ${model}; iOS ${iosVer}; Scale/${scale}) CFNetwork/${cfn} Darwin/${darwin} Alamofire/5.4.3`;
}
...
function buildSignedParamsRaw(capture, overrideDeviceId) {
  ...
  // 签名：所有参数(含signDate，不含sign) 按key排序，拼接后追加SECRET，MD5
...
function buildUrl(path, capture, overrideDeviceId) {
  ...
  return `https://api.wephoneapp.co/app/${path}?${qs}`;
}
...
function genFakeDeviceId() {
  return `${randHex(8)}-${randHex(4)}-${randHex(4)}-${randHex(4)}-${randHex(12)}WePhoneIOS`;
}
...
function buildHeaders(capture, ua) {
  ...
  headers['Host'] = 'api.wephoneapp.co';
...
function runAccount(acc, index, total) {
        ...
        setTimeout(() => {
          ...
          fetchApi('videoBonus', true).then(res => {
            try {
              ...
              if (d.retcode === 0) {
                msgs.push(`🎬 视频${i}：+${d.result?.bonus || '?'} ($${d.result?.aftercredit || '?'})`);
                resolve(next());
              } else {
  ...
  return fetchApi('queryBalanceAndBonus').then(res => {
    try {
      ...
      if (d.retcode === 0) {
        const r = d.result;
        msgs.push(`💰 余额：$${r.balance}`);
        msgs.push(`📋 签到=${r.isallowcheckin ? '可签' : '已签'} 视频=${r.isallowvideobonus ? '可看' : '已满'}`);
    ...
    try {
      ...
      if (d.retcode === 0) {
        const r = d.result;
        msgs.push(`💰 最新余额：$${r.balance}`);
        msgs.push(`📊 今日签到累计：$${r.totalCheckInBonus} 视频累计：$${r.totalVideoBonus}`);
      }
    } catch (e) {}
    return msgs.join('\n');
  }).catch(err => {
...
if (typeof $request !== 'undefined' && $request) {
  ...
  if (!ids.length) {
    notify('⚠️ 未抓到任何账号', '请先打开 WePhone 触发抓包');
    $done();
  } else {
